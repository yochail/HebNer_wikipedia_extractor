...
#-o Output -l --debug --sections --min_text_length 200 --no_templates Data/hewiki-latest-pages-articles.xml.bz2
# =============================================================================
...
class Extractor(object):
    ...
    def extract(self, out):
        ...
        text = compact(text)
        text = [HebNer.tokenize_punctuation(t) for t in text]
        # from zwChan
        #text = [title_str] + text
    ...
    def clean(self, text):
        ...
        text = re.sub('(\[\(«) ', r'\1', text)
     #don't do that - we need that for ner   text = re.sub(r'\n\W+?\n', '\n', text, flags=re.U)  # lines with only punctuations
        text = text.replace(',,', ',').replace(',.', '.')
...
open_delium = "[["
def findBalanced(text, openDelim=[open_delium], closeDelim=[']]']):
    """
    ...
    while True:
        ...
        if not startSet:
            ...
            startSet = True
        delim = next.group(0)
        if startPat.match(delim):#delim in openDelim:
            stack.append(delim)
            nextPat = afterPat[open_delium]
        else:
            opening = stack.pop()
...
def replaceInternalLinks(text):
    ...
    for s, e in findBalanced(text):
        ...
        res += text[cur:s] + makeInternalLink(prefix,title, label,trail)
        cur = end
    return res + text[cur:]
...
def makeInternalLink(prefix,title, label,trail):
    colon = title.find(':')
...
def main():
    ...
    options.num_of_pages = int(args.num_of_pages)
    if args.html:
